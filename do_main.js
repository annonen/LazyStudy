const main = require("./xxqg_v3.1.3");
...
function challengeQuestionLoop(conNum) {
    ...
    {
        console.error("未找到答案或未能成功点击，随机点击");
        let i = random(0, listArray.length - 1);
        listArray[i].child(0).click();//随意点击一个答案
        console.log("------------");
    }
...
function sub() {
    ...
    click("添加");
    delay(2);
    //click("强国号", 0); // text("学习平台").findOne().click() == click("学习平台", 0) 解决订阅问题
    delay(0.5)
    desc("上新").click();
    console.log("请手动添加订阅")
    // let sublist = className("android.view.view").depth(14);
    // var i = 0;
    // while (i < asub) {
...
function main() {
    ...
    var dbName = "/sdcard/tiku/tiku.db";//题库文件名
    if (files.exists(dbName)){
        console.log('已找到外置题库,使用外置题库')
    }else{
        dbName = "./tiku.db";
        console.log('未找到外置题库，使用内置题库')
    }
    console.log("请勿关闭屏幕")
    sleep(500);
    if (now_hours = 12) { //到12点开始运行   
        start_app();//启动app
        ...
        files.copy(path, "/sdcard/tiku/Download/list.db");
        console.warn("自动备份已学文章列表到/sdcard/tiku/Download!!!");
    } 
